...
const getLookupStartOffset = (text: string, offset: number, language?: YomitanLanguage): number => {
    ...
    while (wordStart > 0 && !WHITESPACE_REGEX.test(text[wordStart - 1]) && isWordContinuationAt(text, wordStart - 1)) {
        wordStart -= 1;
    }
    return wordStart;
};
...
export function useTextLookup(source?: { bookId?: string; bookTitle?: string; chapterIndex?: number }) {
    ...
    const getSentenceContext = useCallback(
        (
            ...
            const walker = document.createTreeWalker(contextElement, NodeFilter.SHOW_TEXT, (node) =>
                node.parentElement?.closest('rt, rp') ? NodeFilter.FILTER_REJECT : NodeFilter.FILTER_ACCEPT,
            );
    ...
    const tryLookup = useCallback(
        async (e: React.MouseEvent): Promise<boolean> => {
            ...
            const initialBlockId =
                initialLookupRoot.nodeType === Node.ELEMENT_NODE
