...
export function useHighlights(bookId: string) {
    ...
    const saveHighlights = useCallback(
        async (newHighlights: LNHighlight[]) => {
            try {
    ...
    const addHighlight = useCallback(
        async (chapterIndex: number, blockId: string, text: string, startOffset: number, endOffset: number) => {
    ...
    const exportToJson = useCallback((): string => JSON.stringify(highlights, null, 2), [highlights]);
