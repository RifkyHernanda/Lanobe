...
export function useChapterLoader({ chapters, preloadCount = 2 }: UseChapterLoaderProps): UseChapterLoaderReturn {
    ...
    const isChapterLoaded = useCallback((index: number): boolean => loadedChapters.has(index), [loadedChapters]);

    const getChapterHtml = useCallback(
        (index: number): string | null => loadedChapters.get(index) || null,
        [loadedChapters],
    );
