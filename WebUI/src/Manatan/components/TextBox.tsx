...
export const TextBox: React.FC<{
    ...
    ({
        ...
        useEffect(() => {
            dictPopupRef.current = dictPopup.visible;
        }, [dictPopup.visible]);
        ...
        const bgColor = isDarkTheme ? '#1a1d21' : isWhiteTheme ? '#ffffff' : theme.background;

        // Active Background Logic
        // Dark Theme: slightly lighter dark
        // Other Themes: Theme background (or white if White theme)
        const activeBgColor = isDarkTheme ? '#2d3436' : isWhiteTheme ? '#ffffff' : theme.background;
        ...
        const borderColor = isWhiteTheme || isDarkTheme ? theme.accent : theme.accent;
        ...
        useLayoutEffect(() => {
            ...
            if (!isEditing) {
                const displayTxt = cleanPunctuation(block.text, !isNoSpaceLanguage(settings.yomitanLanguage)).replace(
                    /\u200B/g,
                    '\n',
                );
        ...
        let displayContent = isEditing
            ? block.text
            : cleanPunctuation(block.text, !isNoSpaceLanguage(settings.yomitanLanguage));
        ...
        const getTargetField = useCallback(
            (type: 'Image' | 'Sentence') => {
                if (settings.ankiFieldMap) {
                    const mapped = Object.keys(settings.ankiFieldMap).find(
                        (key) => settings.ankiFieldMap![key] === type,
                    );
        ...
        const updateAnkiCard = useCallback(
        ...
        const confirmAnkiUpdate = useCallback(
            (action: () => void) => {
                ...
                if (results === 'loading') {
                    ...
                    setDictPopup((prev) => ({
                        ...
                        highlight: {
                            ...
                            length: (loadedResults && loadedResults[0]?.matchLen) || 1,
                        },
                    }));
                }
            }
        ...
        const isLookingUp =
            dictPopup.visible && dictPopup.highlight?.imgSrc === imgSrc && dictPopup.highlight?.index === index;
        ...
        const classes = [
            ...
            isLookingUp ? 'active-lookup' : '',
        ]
            .filter(Boolean)
            .join(' ');

        const shouldBeVisible =
            !settings.soloHoverMode || parentVisible || isLocalHover || isEditing || isActive || isLookingUp;
        const menuPosition =
